...
import { Sparkles, Code2, Briefcase, MessageSquare, Send, Github, ExternalLink, User, Heart } from 'lucide-react'
    ...
    <div className="space-y-16 py-6 animate-fade-in">
      ...
      <section className="glass-panel p-8 sm:p-12 rounded-3xl border border-slate-800 space-y-6 relative overflow-hidden">
        <div className="flex flex-col md:flex-row items-center gap-8">
          <div className="w-28 h-28 rounded-2xl bg-gradient-to-tr from-amber-500 via-indigo-500 to-purple-500 p-1 flex-shrink-0 shadow-2xl">
            <img
          ...
          <div className="space-y-3 text-center md:text-left">
            ...
            <p className="text-slate-400 text-sm leading-relaxed max-w-2xl">
              热衷于全栈技术架构、前端极致 UI/UX 体验以及自主 AI 智能体应用。坚信优雅的代码与精致的设计相得益彰。
            </p>
      ...
      <section className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {projects.map((proj) => (
            <div
              key={proj.id}
              className="glass-panel p-6 rounded-2xl border border-slate-800 hover:border-amber-500/40 transition-all flex flex-col justify-between group"
            >
        ...
        <div className="relative pl-6 space-y-8 before:absolute before:left-2.5 before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-800">
          {timeline.map((item, index) => (
            <div key={index} className="relative group">
              <div className="absolute -left-6 top-1.5 w-5 h-5 rounded-full bg-slate-950 border-2 border-indigo-500 group-hover:scale-125 transition-transform" />
      ...
      <section className="space-y-6">
        <div className="flex items-center space-x-3">
          <div className="w-2.5 h-6 rounded-full bg-rose-500" />
          <h2 className="text-2xl font-bold text-slate-100 flex items-center space-x-2">
        ...
        <form onSubmit={handleSubmitComment} className="glass-panel p-6 rounded-2xl border border-slate-800 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <input
              ...
              className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-xs text-slate-200 focus:outline-none focus:border-rose-500/60"
              required
            />
          </div>
          ...
          <div className="flex justify-end">
            <button
              type="submit"
              className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-rose-600 to-amber-600 text-white font-medium text-xs hover:from-rose-500 hover:to-amber-500 transition-all flex items-center space-x-1.5 shadow-lg shadow-rose-600/20"
            >
