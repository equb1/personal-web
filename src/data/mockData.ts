...
export const SAMPLE_BOOKS: Book[] = [
  {
    ...
    excerpt: [
      '# 第一章：日用品中的设计心理学（原文章节节选）',
      '',
      '当你面对一扇门，却不知道该推还是拉时，这绝不是你的智力问题，而是**门的设计存在严重缺陷**。',
      '',
      '### 1.1 示能 (Affordances)',
      '“示能”指的是一个物理对象与人之间的基本交互属性。例如，一个平坦的金属推板具有“可推”的示能；而一个凸起的把手则具有“可拉”的示能。如果一扇门装有把手却只能推，使用者就会本能地拉动它并产生挫败感。',
      '',
      '### 1.2 意符 (Signifiers)',
      '意符是任何可以被感知到的信号，告诉用户该执行什么操作以及在哪里操作。在数字产品界面中，按钮的立体阴影、文本下划线都是典型的意符。',
      '',
      '### 1.3 概念模型与即时反馈 (Feedback)',
      '每一项操作都必须提供即时且明确的反馈信号。如果用户点击了提交按钮，界面在两秒内没有任何 Loading 或响应指示，用户就会怀疑操作是否生效并重复点击。'
    ].join('\n'),
    thoughts: [
      '# 深度读书笔记：《设计心理学》的工程实践启示',
      '',
      '优秀的设计是隐形的。用户在使用工具时遇到的困惑，责任永远在产品与前端工程师身上，而非用户。',
      '',
      '## 💡 映射到 Web 前端开发的原则：',
  ...
  },
  {
    ...
    publishYear: '2016',
    pages: 740,
    spineColor: 'from-blue-700 to-indigo-800',
    summary: '从程序员的视角探索计算机系统，涵盖汇编、内存层级、虚拟内存与并发编程。',
    tags: ['C语言', '操作系统', '体系结构'],
    excerpt: [
      '# 第 6 章：存储器层次结构（原文章节节选）',
      '',
      '### 6.2 局部性原理 (Principle of Locality)',
      '一个编写良好的计算机程序通常倾向于展现出良好的局部性：',
      '1. **时间局部性 (Temporal Locality)**：被引用过一次的内存位置很可能在不远的将来再次被引用多次。',
      '2. **空间局部性 (Spatial Locality)**：如果一个内存位置被引用了一次，那么程序很可能在不远的将来引用其附近的一个内存位置。',
      '',
      '### 6.3 存储器层次结构中的缓存',
      '存储器层次结构的核心思想在于：对于每个 $k$，位于第 $k$ 层的更快更小的存储设备，作为第 $k+1$ 层更大更慢设备的缓存。从寄存器到 L1/L2/L3 Cache、主存、固态硬盘，访问延迟呈指数级攀升。'
    ].join('\n'),
    thoughts: [
      '# 深度读书笔记：CSAPP 内存层级与 Cache 优化',
      '',
      '内存山（Memory Mountain）完美展示了 L1/L2 Cache 命中的巨大性能优势。',
      '',
      '## 💡 映射到前端性能调优：',
  ...
  },
  {
    ...
    publishYear: '2018',
    pages: 412,
    spineColor: 'from-emerald-700 to-teal-800',
    summary: '为软件工程师量身定制的个人品牌建立、时间管理与终身学习法则。',
    tags: ['个人成长', '时间管理', '生产力'],
    excerpt: [
      '# 第 18 章：建立个人品牌与公开学习（原文章节节选）',
      '',
      '大多数程序员花费 100% 的时间在敲代码上，而花费 0% 的时间让别人知道自己的存在。',
      '',
      '### 为什么你应该写技术博客？',
      '写博客并不是为了证明你有多聪明，而是为了：',
      '1. **将模糊的概念固化**：教别人是最高效的学习方式（费曼学习法）。',
      '2. **建立数字花园**：让你的思考、阅读笔记与项目成果在互联网上 24 小时为你发声。',
